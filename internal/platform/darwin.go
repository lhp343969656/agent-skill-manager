...
// DarwinLinker 实现 macOS 平台的链接策略
// 优先级：Symlink -> 托管复制
type DarwinLinker struct{}
...
// Create 创建链接，按 Symlink -> Copy 顺序尝试
func (l *DarwinLinker) Create(source, target string) (LinkMode, error) {
	if source == "" || target == "" {
...
func (l *DarwinLinker) Inspect(target string) (LinkInfo, error) {
	return inspectCommon(target)
}

// RemoveManaged 移除由管理器创建的链接
func (l *DarwinLinker) RemoveManaged(target, expectedSource string) error {
